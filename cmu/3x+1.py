...
app.xData=[]
app.yData=[]
initial=app.getTextInput("write any number")
app.number=initial
l=Label(app.number,200,200,size=40)
app.i=0
app.done=False
Label(initial, 380,20,size=20)
Label('3x+1 visualized',215,20,size=14)
Label('if a number is even it is divided by 2 and if it is ', 215, 34, size=10)
Label('odd it is multiplied by 3 and 1 gets added to it',215,44,size=10)
...
def onStep():
    if(int(app.number)>1):  
        ...
        if(int(app.number)%2==0):
            app.number=int(app.number)/2
            l.value=app.number
        else:
            app.number=int(app.number)*3
            app.number+=1
            l.value=app.number
        app.yData.append(l.value)
        app.xData.append(app.i)
    if(int(app.number)==1):
...
def main():
    ...
    for i in range(app.i):
        xData.append(app.xData[i])
        yData.append(app.yData[i])
    manager.plotLines(xData, yData)
    
    app.stop
manager = PlotManager(left=50, bottom=370, width=330, height=330)
